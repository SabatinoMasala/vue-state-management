    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div class="flex h-16 justify-between">
        <div class="flex items-center">
          <NuxtLink to="/" class="font-bold relative flex bg-white p-1 text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2">
            Acme store
          </NuxtLink>
        </div>
        <div class="flex items-center ml-auto">
          <div class="md:ml-4 md:flex md:shrink-0 md:items-center">
            <template v-if="!userStore.user">
              <NuxtLink to="/login" class="relative flex bg-white p-1 text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2">
                Login
              </NuxtLink>
              <NuxtLink to="/register" class="relative flex bg-white p-1 text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2">
                Register
              </NuxtLink>
            </template>
            <template v-else>
              <NuxtLink data-testid="username" to="/profile" class="relative flex bg-white p-1 text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-slate-500 focus:ring-offset-2">
                {{ userStore.user.name }}
              </NuxtLink>
            </template>
